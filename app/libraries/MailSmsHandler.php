...
class MailSmsHandler {
	...
	public function custom($to,$message){
		if($this->settings['customHTTPURL'] == "" || $this->settings['customHTTPParams'] == "" || $this->settings['customHTTPParams'] == "") return;
		$ch = curl_init();
		...
		$this->settings['customHTTPParams'] = str_replace("{to}",$to,$this->settings['customHTTPParams']);
		$this->settings['customHTTPParams'] = str_replace("{message}",$message,$this->settings['customHTTPParams']);
	...
	public function enterpriseliveair($to,$message){
		...
		file_get_contents("http://enterprise.liveair.in/httpapi/smsapi?uname=".$this->settings['enterpriseliveairUserName']."&password=".$this->settings['enterpriseliveairPassword']."&sender=".$this->settings['enterpriseliveairSenderId']."&receiver=".$to."&route=T&msgtype=1&sms=".urlencode($message));
	}
