...
<html>
  ...
  <body class="hold-transition <?php echo $panelInit->defTheme; ?> sidebar-mini fixed sidebar-collapse" ng-app="schoex" ng-controller="mainController">
    <div class="wrapper">
              ...
              while (list($key, $value) = each($panelInit->panelItems)) {
                  ...
                  if(!in_array($users->role, $value['permissions'])){
				   ...
				   if($users->role == "teacher"){
						if($value['title'] == "Staff Attendance"){
								
								  // $newrole=html_entity_decode($users->newrole);
								  $newrole=json_decode($users->newrole);
							     if(!array_intersect($newrole, $staffattendancepermission)){
                                        continue;
                                    }
							   }
 }	
				 ...
				 echo "<a ";
                  ...
                  if(isset($value['children'])){
                      ...
                      while (list($key2, $value2) = each($value['children'])) {
                         ...
                         if($users->role == "teacher"){
						if($value2['title'] == "Attendance"){
								
								  // $newrole=html_entity_decode($users->newrole);
								  $newrole=json_decode($users->newrole);
							     if(!array_intersect($newrole, $attendancepermission)){
                                        continue;
                                    }
							   }
 }							   
