...
class LoginController extends BaseController {
	...
	public function customattemp()
	{
		if (filter_var(Input::get('email'), FILTER_VALIDATE_EMAIL)) {
			if (Auth::attempt(array('email' => Input::get('email'), 'password' => Input::get('password'),'activated'=>1,'role'=>Input::get('role')),Input::get('remember_me')))
			{
				return Redirect::to('/');
			}else{
				return Redirect::to('/login')->withErrors(array($this->panelInit->language['chkUserPass']));
			...
			if (Auth::attempt(array('username' => Input::get('email'), 'password' => Input::get('password'),'activated'=>1,'role'=>Input::get('role')),Input::get('remember_me')))
			{
				return Redirect::to('/');
			}else{
				return Redirect::to('/login')->withErrors(array($this->panelInit->language['chkUserPass']));
